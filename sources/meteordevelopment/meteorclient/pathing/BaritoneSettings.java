...
public class BaritoneSettings implements IPathManager.ISettings {
    ...
    static {
        SETTING_MAX_VALUES.put("pathCutoffFactor", Double.valueOf(1.0d));
    }
                ...
                if (!Modifier.isStatic(field.getModifiers())) {
                    ...
                    if (obj instanceof Settings.Setting) {
                        ...
                        if (value instanceof Boolean) {
                            wrapper = sgBool.add(new BoolSetting.Builder().name(setting.getName()).description(getDescription(setting.getName())).defaultValue(Boolean.valueOf(((Boolean) setting.defaultValue).booleanValue())).onChanged(aBoolean -> {
                            ...
                            sgDouble.add(new DoubleSetting.Builder().name(setting.getName()).description(getDescription(setting.getName())).defaultValue(((Double) setting.defaultValue).doubleValue()).max(SETTING_MAX_VALUES.getOrDefault(setting.getName(), Double.valueOf(10.0d)).doubleValue()).sliderMax(SETTING_MAX_VALUES.getOrDefault(setting.getName(), Double.valueOf(10.0d)).doubleValue()).onChanged(aDouble -> {
                            ...
                            sgDouble.add(new DoubleSetting.Builder().name(setting.getName()).description(getDescription(setting.getName())).defaultValue(((Float) setting.defaultValue).doubleValue()).max(SETTING_MAX_VALUES.getOrDefault(setting.getName(), Double.valueOf(10.0d)).doubleValue()).sliderMax(SETTING_MAX_VALUES.getOrDefault(setting.getName(), Double.valueOf(10.0d)).doubleValue()).onChanged(aDouble2 -> {
                                setting.value = Float.valueOf(aDouble2.floatValue());
                            }).onModuleActivated(doubleSetting2 -> {
                                doubleSetting2.set(Double.valueOf(((Float) setting.value).doubleValue()));
                            }).build());
                        } else if (value instanceof Integer) {
                            Setting<Integer> wrapper2 = sgInt.add(new IntSetting.Builder().name(setting.getName()).description(getDescription(setting.getName())).defaultValue(Integer.valueOf(((Integer) setting.defaultValue).intValue())).onChanged(integer -> {
                                setting.value = integer;
                            }).onModuleActivated(integerSetting -> {
                                integerSetting.set((Integer) setting.value);
                            }).build());
                            if (wrapper2.name.equals("maxFallHeightNoWater")) {
                                this.noFall = new BoolSetting.Builder().name(wrapper2.name).description(wrapper2.description).defaultValue(false).onChanged(aBoolean2 -> {
                                    wrapper2.set(Integer.valueOf(aBoolean2.booleanValue() ? 159159 : ((Integer) wrapper2.getDefaultValue()).intValue()));
                                }).onModuleActivated(booleanSetting2 -> {
                                    booleanSetting2.set(Boolean.valueOf(((Integer) wrapper2.get()).intValue() >= 255));
                                }).build();
                            }
                        } else if (value instanceof Long) {
                            sgInt.add(new IntSetting.Builder().name(setting.getName()).description(getDescription(setting.getName())).defaultValue(Integer.valueOf(((Long) setting.defaultValue).intValue())).onChanged(integer2 -> {
                                setting.value = Long.valueOf(integer2.longValue());
                            }).onModuleActivated(integerSetting2 -> {
                                integerSetting2.set(Integer.valueOf(((Long) setting.value).intValue()));
                            }).build());
                        } else if (value instanceof String) {
                            sgString.add(new StringSetting.Builder().name(setting.getName()).description(getDescription(setting.getName())).defaultValue((String) setting.defaultValue).onChanged(string -> {
                            ...
                            sgColor.add(new ColorSetting.Builder().name(setting.getName()).description(getDescription(setting.getName())).defaultValue(new SettingColor(c.getRed(), c.getGreen(), c.getBlue(), c.getAlpha())).onChanged(color -> {
                            ...
                            if (type == class_2248.class) {
                                sgBlockLists.add(new BlockListSetting.Builder().name(setting.getName()).description(getDescription(setting.getName())).defaultValue((List) setting.defaultValue).onChanged(blockList -> {
                                ...
                                sgItemLists.add(new ItemListSetting.Builder().name(setting.getName()).description(getDescription(setting.getName())).defaultValue((List) setting.defaultValue).onChanged(itemList -> {

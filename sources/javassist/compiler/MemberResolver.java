...
/* JADX INFO: loaded from: meteor-client-1.21.5-local.jar:javassist/compiler/MemberResolver.class */
public class MemberResolver implements TokenId {
    ...
    /* JADX INFO: loaded from: meteor-client-1.21.5-local.jar:javassist/compiler/MemberResolver$Method.class */
    ...
    public Method lookupMethod(CtClass clazz, CtClass currentClass, MethodInfo current, String methodName, int[] argTypes, int[] argDims, String[] argClassNames) throws CompileError {
        int res;
        Method maybe = null;
        if (current != null && clazz == currentClass && current.getName().equals(methodName) && (res = compareSignature(current.getDescriptor(), argTypes, argDims, argClassNames)) != NO) {
    ...
    private Method lookupMethod(CtClass clazz, String methodName, int[] argTypes, int[] argDims, String[] argClassNames, boolean onlyExact) throws CompileError {
        ...
        if (cf != null) {
            ...
            for (MethodInfo minfo : list) {
                if (minfo.getName().equals(methodName) && (minfo.getAccessFlags() & 64) == 0 && (res = compareSignature(minfo.getDescriptor(), argTypes, argDims, argClassNames)) != NO) {

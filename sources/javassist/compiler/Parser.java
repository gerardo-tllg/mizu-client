...
/* JADX INFO: loaded from: meteor-client-1.21.5-local.jar:javassist/compiler/Parser.class */
public final class Parser implements TokenId {
    ...
    private Stmnt parseTry(SymbolTable tbl) throws CompileError {
        ...
        ASTList aSTListAppend = null;
        while (true) {
            ASTList catchList = aSTListAppend;
            if (this.lex.lookAhead() == 305) {
                ...
                aSTListAppend = ASTList.append(catchList, new Pair(d, b));
            } else {
    ...
    private Stmnt parseDeclarationOrExpression(SymbolTable tbl, boolean exprList) throws CompileError {
        ...
        Stmnt expr;
        int i;
        int iLookAhead = this.lex.lookAhead();
        while (true) {
            t = iLookAhead;
            if (t != 315) {
                break;
    ...
    private ASTree parseBinaryExpr(SymbolTable tbl) throws CompileError {
        ASTree unaryExpr = parseUnaryExpr(tbl);
        while (true) {
            ASTree expr = unaryExpr;
            int t = this.lex.lookAhead();
            int p = getOpPrecedence(t);
            if (p == 0) {
                return expr;
            }
            unaryExpr = binaryExpr2(tbl, expr, p);
        }
    }
    ...
    private ASTree binaryExpr2(SymbolTable tbl, ASTree expr, int prec) throws CompileError {
        ...
        while (true) {
            ...
            unaryExpr = binaryExpr2(tbl, expr2, p2);
        }
    ...
    */
    ...
    private ASTList parseArraySize(SymbolTable tbl) throws CompileError {
        ASTList aSTListAppend = null;
        while (true) {
            ASTList list = aSTListAppend;
            if (this.lex.lookAhead() == 91) {
                aSTListAppend = ASTList.append(list, parseArrayIndex(tbl));
            } else {
                return list;
            }
        }

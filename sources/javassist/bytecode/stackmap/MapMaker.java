...
/* JADX INFO: loaded from: meteor-client-1.21.5-local.jar:javassist/bytecode/stackmap/MapMaker.class */
public class MapMaker extends Tracer {
    ...
    private void fixTypes(byte[] code, TypedBlock[] blocks) throws BadBytecode, NotFoundException {
    ...
    public StackMap toStackMap2(ConstPool cp, TypedBlock[] blocks) {
        ...
        for (int i = 1; i < n; i++) {
            TypedBlock bb = blocks[i];
            boolean zIsTarget = isTarget(bb, blocks[i - 1]);
            effective[i] = zIsTarget;
            if (zIsTarget) {
                bb.resetNumLocals();
                num++;
            }

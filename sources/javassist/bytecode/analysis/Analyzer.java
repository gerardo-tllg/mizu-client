...
/* JADX INFO: loaded from: meteor-client-1.21.5-local.jar:javassist/bytecode/analysis/Analyzer.class */
public class Analyzer implements Opcode {
    ...
    /* JADX INFO: loaded from: meteor-client-1.21.5-local.jar:javassist/bytecode/analysis/Analyzer$ExceptionInfo.class */
    ...
    private void mergeRet(IntQueue queue, CodeIterator iter, int pos, Frame frame, Subroutine subroutine) throws BadBytecode {
        ...
        while (it.hasNext()) {
            int caller = it.next().intValue();
            int returnLoc = getNext(iter, caller, pos);
            Frame old = this.frames[returnLoc];
            if (old == null) {
                Frame[] frameArr = this.frames;
                Frame frameCopyStack = frame.copyStack();
                frameArr[returnLoc] = frameCopyStack;
                old = frameCopyStack;
                changed = true;
            } else {

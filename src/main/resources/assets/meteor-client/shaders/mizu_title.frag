...
float waveSurf(float px,float tm,float scale){
  ...
  float ripple=sin(px*22.0-tm*1.8)*0.008*scale+sin(px*35.0+tm*2.2)*0.005*scale;
  float turb=fbm(vec2(px*2.2+slow*0.3,slow*0.15))*0.018*scale;
...
void main(){
  ...
  vec3 lineCol1=vec3(0.04,0.38,0.30);
  vec3 lineCol2=vec3(0.05,0.32,0.26);
  vec3 lineCol3=vec3(0.04,0.26,0.22);
  vec3 lineCol4=vec3(0.035,0.20,0.18);
  vec3 lineCol5=vec3(0.028,0.15,0.14);
  vec3 col=bg;

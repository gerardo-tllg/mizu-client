...
// import meteordevelopment.meteorclient.mixin.BufferRendererAccessor; // Removed in 1.21.5
// import meteordevelopment.meteorclient.mixininterface.ICapabilityTracker; // Removed - no longer needed in 1.21.5
// import net.fabricmc.loader.api.FabricLoader; // Removed - no longer needed
import net.minecraft.util.Identifier;
import org.joml.Matrix4f;
import org.lwjgl.BufferUtils;

// import java.lang.reflect.Field; // Removed - no longer needed
import java.nio.ByteBuffer;
import java.nio.FloatBuffer;
// import java.util.List; // Removed - no longer needed
...
public class GL {
    ...
    // Capability trackers removed in 1.21.5 - no longer needed as saveState/restoreState use glIsEnabled directly
    ...
    public static void deleteVertexArray(int vao) {
        // In 1.21.5, _glDeleteVertexArrays removed - use direct GL call
    ...
    public static void bindVertexArray(int vao) {
        ...
        // BufferRendererAccessor.setCurrentVertexBuffer(null); // Removed in 1.21.5
    }
    ...
    public static void shaderSource(int shader, String source) {
        // In 1.21.5, glShaderSource expects String, not List
    ...
    public static void uniformMatrix(int location, Matrix4f v) {
        v.get(MAT);
        // In 1.21.5, use direct GL call for matrix uniform
    ...
    public static void saveState() {
        // In 1.21.5, capability tracker get() method changed
        depthSaved = glIsEnabled(GL_DEPTH_TEST);
        blendSaved = glIsEnabled(GL_BLEND);
        cullSaved = glIsEnabled(GL_CULL_FACE);
        scissorSaved = glIsEnabled(GL_SCISSOR_TEST);
    }

    public static void restoreState() {
        // In 1.21.5, capability tracker set() method changed
        if (depthSaved) enableDepth(); else disableDepth();
        if (blendSaved) enableBlend(); else disableBlend();
        if (cullSaved) enableCull(); else disableCull();
        if (scissorSaved) enableScissorTest(); else disableScissorTest();
    ...
    public static void enableBlend() {
        ...
        // In 1.21.5, use direct GL call for blendFunc
    ...
    public static void bindTexture(Identifier id) {
        ...
        // In 1.21.5, bindTexture() was removed - get GlId and bind directly
        net.minecraft.client.texture.AbstractTexture texture = mc.getTextureManager().getTexture(id);
        if (texture.getGlTexture() instanceof net.minecraft.client.texture.GlTexture glTexture) {
    ...
    // getTracker method removed - no longer needed in 1.21.5
    // Capability tracking is now handled directly via glIsEnabled() in saveState/restoreState
    /*

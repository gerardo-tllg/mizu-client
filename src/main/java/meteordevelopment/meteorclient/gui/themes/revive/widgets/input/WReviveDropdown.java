...
public class WReviveDropdown<T> extends WDropdown<T> implements ReviveWidget {
    public WReviveDropdown(T[] values, T value) { super(values, value); }

    @Override
    protected WDropdownRoot createRootWidget() { return new WRoot(); }

    @Override
    protected WDropdownValue createValueWidget() { return new WValue(); }
    ...
    protected void onRender(GuiRenderer renderer, double mouseX, double mouseY, double delta) {
        ...
        renderer.rotatedQuad(x + pad + maxValueWidth + pad, y + pad, s, s, 0, GuiRenderer.TRIANGLE, theme.accentColor.get());
    }

    private static class WRoot extends WDropdownRoot implements ReviveWidget {
        @Override
        protected void onRender(GuiRenderer renderer, double mouseX, double mouseY, double delta) {
            ReviveGuiTheme theme = theme();
    ...
    private class WValue extends WDropdownValue implements ReviveWidget {
        @Override

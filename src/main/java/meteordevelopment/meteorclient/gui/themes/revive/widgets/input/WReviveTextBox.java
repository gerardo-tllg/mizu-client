...
public class WReviveTextBox extends WTextBox implements ReviveWidget {
    ...
    public WReviveTextBox(String text, String placeholder, CharFilter filter, Class<? extends Renderer> renderer) {
    ...
    protected WContainer createCompletionsRootWidget() {
        return new WVerticalList() {
            ...
            protected void onRender(GuiRenderer renderer1, double mouseX, double mouseY, double delta) {
                ReviveGuiTheme theme1 = theme();
                double s = theme1.scale(1);
    ...
    private static class CompletionItem extends WReviveLabel implements ICompletionItem {
        private static final Color SELECTED_COLOR = new Color(0, 130, 255, 25);
        ...
        @Override public String getCompletion() { return text; }
    }
    ...
    protected void onRender(GuiRenderer renderer, double mouseX, double mouseY, double delta) {
        ...
        if ((focused && cursorVisible) || animProgress > 0) {
            ...
            renderer.quad(x + pad + getTextWidth(cursor) - overflowWidth, y + pad, theme.scale(1), theme.textHeight(), theme.accentColor.get());
            renderer.setAlpha(1);
        }

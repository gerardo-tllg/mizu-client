...
public class AutoTool extends Module {
    ...
    private void onStartBreakingBlock(StartBreakingBlockEvent event) {
        ...
        if (bestSlot != -1 && bestScore > getScore(currentStack, blockState, silkTouchForEnderChest.get(), fortuneForOresCrops.get(), prefer.get(), itemStack -> !shouldStopUsing(itemStack))
    ...
    public static double getScore(ItemStack itemStack, BlockState state, boolean silkTouchEnderChest, boolean fortuneOre, EnchantPreference enchantPreference, Predicate<ItemStack> good) {
        ...
        // A tool is valid if it can mine this block efficiently, or falls into a special case:
        // - Swords on SWORD_EFFICIENT blocks (cobweb, bamboo, etc.)
        // - Shears on leaves
        // - Any tool on a SWORD_EFFICIENT tagged block
        if (!itemStack.isSuitableFor(state)
            && !(itemStack.isIn(ItemTags.SWORDS) && (state.getBlock() instanceof CobwebBlock || state.isIn(BlockTags.SWORD_EFFICIENT)))
        ...
        // Give swords a major priority bonus when mining blocks they excel at (cobweb, bamboo, etc.)
        if (itemStack.isIn(ItemTags.SWORDS) && state.isIn(BlockTags.SWORD_EFFICIENT)) {
            score += 9000.0F + itemStack.getMiningSpeedMultiplier(state) * 1000.0F;
        }
    ...
    public static boolean isTool(Item item) {
        return item.getDefaultStack().contains(DataComponentTypes.TOOL) || item instanceof ShearsItem;
    }

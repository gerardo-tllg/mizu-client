...
public class InstantRebreak extends Module {
    ...
    public void sendPacket() {
        mc.getNetworkHandler().sendPacket(new PlayerActionC2SPacket(PlayerActionC2SPacket.Action.ABORT_DESTROY_BLOCK, blockPos, direction == null ? Direction.UP : direction));
    }

    public boolean shouldMine() {
        if (!mc.world.isOutOfHeightLimit(blockPos) && BlockUtils.canBreak(blockPos)) {
            return !pick.get() || mc.player.getMainHandStack().contains(DataComponentTypes.TOOL);
        } else {
            return false;
        }
    }
    ...
    private void onRender(Render3DEvent event) {
        if (render.get() && shouldMine()) {
            event.renderer.box(blockPos, (Color) sideColor.get(), (Color) lineColor.get(), shapeMode.get(), 0);
        }
    }
}

...
/**
 * WheelPicker - GTA-style radial menu for quick module toggles, messages, and commands
 * 
 * Adapted from BepHaxAddon by dekrom
...
public class WheelPicker extends Module {
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        // Don't activate if screen is open
    ...
    private void updateSelectedSlot() {
        ...
        double centerX = scaledWidth / 2.0 + wheelX.get();
        double centerY = scaledHeight / 2.0 + wheelY.get();
        ...
        // Offset by 90 degrees (top is 0)
        degrees = (degrees + 90) % 360;
        
        // Determine slot (8 segments of 45 degrees each)
    ...
    private void sendMessageWithProtection(String message) {
        if (spamProtection.get()) {
            ...
            // Add invisible chars to bypass spam detection
    ...
    private String applyRandomSubstitution(String text) {
        ...
        // Insert random text in existing brackets
    ...
    private void onRender2D(Render2DEvent event) {
        ...
        // Blending is enabled by default in DrawContext
    ...
    private void renderWheel(DrawContext context, int centerX, int centerY, int radius) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            drawThickLine(context, centerX, centerY, x1, y1, borderColor.get().getPacked(), 2);
        }
    ...
    private void drawFilledCircle(DrawContext context, int centerX, int centerY, int radius, SettingColor color) {
    ...
    private void drawWheelSection(DrawContext context, int centerX, int centerY, int radius, int sectionIndex, boolean selected) {
        ...
        SettingColor color = selectedColor.get();
        int segments = 45;
    ...
    private void drawSectionLabel(DrawContext context, int centerX, int centerY, int radius, int sectionIndex) {
        ...
        // Draw text
        if (hasText) {
            SettingColor textCol = isModuleActive ? moduleActiveColor.get() : this.textColor.get();

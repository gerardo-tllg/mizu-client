...
import meteordevelopment.meteorclient.systems.modules.movement.elytrafly.modes.*;
...
public class ElytraFly extends Module {
    ...
    private void onPlayerMove(PlayerMoveEvent event) {
        ...
        if (mc.player.isGliding()) {
            ...
            if (dontGoIntoUnloadedChunks.get()) {
                if (mc.world.getChunkManager().isChunkLoaded(chunkX, chunkZ)) {
                    if ((flightMode.get() != ElytraFlightModes.Bounce && flightMode.get() != ElytraFlightModes.Slide)) ((IVec3d) event.movement).meteor$set(currentMode.velX, currentMode.velY, currentMode.velZ);
                } else {
                    currentMode.zeroAcceleration();
                    ((IVec3d) event.movement).meteor$set(0, currentMode.velY, 0);
                }
            } else if ((flightMode.get() != ElytraFlightModes.Bounce && flightMode.get() != ElytraFlightModes.Slide)) ((IVec3d) event.movement).meteor$set(currentMode.velX, currentMode.velY, currentMode.velZ);
        ...
        if (autoHover.get() && mc.player.isSneaking() && !Modules.get().get(Freecam.class).isActive() && mc.player.isGliding() && (flightMode.get() != ElytraFlightModes.Bounce && flightMode.get() != ElytraFlightModes.Slide)) {
            ...
            if (underCollidable) {
                ...
                if (mc.player.getPos().y <= mc.player.getBlockPos().down().getY() + 1.34f) {
                    ...
                    // In 1.21.5, input.sneaking removed - use setSneaking
                }
            }
        }
    }
    ...
    public boolean canPacketEfly() {
        return isActive() && flightMode.get() == ElytraFlightModes.Packet && mc.player.getEquippedStack(EquipmentSlot.CHEST).getItem() == Items.ELYTRA && !mc.player.isOnGround();
    }
    ...
    private class StaticInstaDropListener {
        ...
        private void onInstadropTick(TickEvent.Post event) {
            if (mc.player != null && mc.player.isGliding()) {
                ...
                // In 1.21.5, OnGroundOnly constructor requires horizontalCollision parameter

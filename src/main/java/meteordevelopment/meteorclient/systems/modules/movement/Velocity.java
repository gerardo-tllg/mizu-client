...
public class Velocity extends Module {
    ...
    private void onTick(TickEvent.Post event) {
        if (sinking.get() && !mc.options.jumpKey.isPressed() && !mc.options.sneakKey.isPressed()
            && (mc.player.isTouchingWater() || mc.player.isInLava()) && mc.player.getVelocity().y < 0.0) {
            ((IVec3d) mc.player.getVelocity()).meteor$setY(0.0);
        }
    }
        ...
        if (knockback.get()) {
            if (event.packet instanceof EntityVelocityUpdateS2CPacket packet
                ...
                if (knockbackPhaseOnly.get()) {
                    if (knockbackPhaseInAir.get() && !RotationManager.lastGround) return;
                    if (!PlayerUtils.isPlayerPhased()) return;
                }

                double velX = (packet.getVelocityX() / 8000.0 - mc.player.getVelocity().x) * knockbackHorizontal.get();
                double velY = (packet.getVelocityY() / 8000.0 - mc.player.getVelocity().y) * knockbackVertical.get();
                double velZ = (packet.getVelocityZ() / 8000.0 - mc.player.getVelocity().z) * knockbackHorizontal.get();
                ((EntityVelocityUpdateS2CPacketAccessor) packet).setX((int) (velX * 8000.0 + mc.player.getVelocity().x * 8000.0));
    ...
    public double getHorizontal(Setting<Double> setting) {
        return isActive() ? setting.get() : 1.0;
    }

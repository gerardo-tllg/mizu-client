...
public class ElytraFakeFly extends Module {
    ...
    private void onPlayerMove(PlayerMoveEvent event) {
        ...
        ((IVec3d) event.movement).meteor$set(newMovement.x, newMovement.y, newMovement.z);
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        if (event.packet instanceof PlayerPositionLookS2CPacket packet) {
            // In 1.21.5, packet structure changed - position in PlayerPosition record
            // Using public record accessor methods
            Vec3d position = packet.change().position();
            
            if (packet.relatives().contains(PositionFlag.X)) {
                currentVelocity = new Vec3d(position.x, currentVelocity.y, currentVelocity.z);
            }

            if (packet.relatives().contains(PositionFlag.Y)) {
                currentVelocity = new Vec3d(currentVelocity.x, position.y, currentVelocity.z);
            }

            if (packet.relatives().contains(PositionFlag.Z)) {
                currentVelocity = new Vec3d(currentVelocity.x, currentVelocity.y, position.z);
            }
            ...
            if (!packet.relatives().contains(PositionFlag.X)
                ...
                if (System.currentTimeMillis() - timeOfLastRubberband < 100) {
                    currentVelocity = position.subtract(lastRubberband);
                }

                timeOfLastRubberband = System.currentTimeMillis();
                lastRubberband = position;
            }
        }
    }

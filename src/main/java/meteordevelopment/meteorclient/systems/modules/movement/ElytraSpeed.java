...
public class ElytraSpeed extends Module {
    ...
    private void onPacketReceive(PacketEvent.Receive event) {
        ...
        if (event.packet instanceof PlayerPositionLookS2CPacket lookS2CPacket) {
            ...
            // In 1.21.5, packet structure changed - yaw/pitch are in PlayerPosition record
            // Using public record accessor methods
            yaw = Math.toRadians(lookS2CPacket.change().yaw());
            pitch = Math.toRadians(lookS2CPacket.change().pitch());
        }
    }
    ...
    public void onPlayerMove(PlayerMoveEvent event) {
        ...
        if (!rubberband && (!using || !mc.player.isGliding() || eFly.isActive() || geFly.isActive())) {

...
public class ElytraFlightMode {
    ...
    public void onTick() {
        ...
        if (elytraFly.replace.get()) {
            ItemStack chestStack = ((IPlayerInventory) mc.player.getInventory()).meteor$getArmor().get(2);

            if (chestStack.getItem() == Items.ELYTRA && chestStack.getMaxDamage() - chestStack.getDamage() <= elytraFly.replaceDurability.get()) {
                FindItemResult elytra = InvUtils.find(stack ->
                    stack.getMaxDamage() - stack.getDamage() > elytraFly.replaceDurability.get() && stack.getItem() == Items.ELYTRA
                );
    ...
    public void handleAutopilot() {
        if (mc.player.isGliding()) {
            ...
            if (elytraFly.useFireworks.get()) {
                if (ticksLeft <= 0.0) {
                    ticksLeft = elytraFly.autoPilotFireworkDelay.get() * 20.0;
    ...
    public void handleHorizontalSpeed(PlayerMoveEvent event) {
        ...
        if (mc.options.forwardKey.isPressed()) {
            velX += forward.x * getSpeed() * 10.0;
            velZ += forward.z * getSpeed() * 10.0;
            a = true;
        ...
        if (mc.options.rightKey.isPressed()) {
            velX += right.x * getSpeed() * 10.0;
            velZ += right.z * getSpeed() * 10.0;
            b = true;
    ...
    public void handleFallMultiplier() {
        if (velY < 0.0) velY *= elytraFly.fallMultiplier.get();
        else if (velY > 0.0) velY = 0.0;
    }

    public void handleAcceleration() {
        if (elytraFly.acceleration.get()) {
            if (!PlayerUtils.isMoving()) acceleration = 0.0;
            acceleration = Math.min(
                acceleration + elytraFly.accelerationMin.get() + elytraFly.accelerationStep.get() * 0.1,

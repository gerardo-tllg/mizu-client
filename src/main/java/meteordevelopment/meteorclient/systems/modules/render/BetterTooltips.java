...
public class BetterTooltips extends Module {
    ...
    private void appendTooltip(ItemStackTooltipEvent event) {
        ...
        if (statusEffects.get()) {
            if (event.itemStack().getItem() == Items.SUSPICIOUS_STEW) {
                ...
                // FoodComponent effects API changed in 1.21.5 - commenting out for now
                /*if (food != null && !food.effects().isEmpty()) {
                    for (var e : food.effects()) {
                        event.appendStart(getStatusText(e.effect()));
                    }
                }*/
            }
        }
        ...
        if (beehive.get()) {
            if (event.itemStack().getItem() == Items.BEEHIVE || event.itemStack().getItem() == Items.BEE_NEST) {
                ...
                var bees = event.itemStack().get(DataComponentTypes.BEES);
                if (bees != null) {
                    // BeesComponent API changed in 1.21.5
                    event.appendStart(Text.literal(String.format("%sBees: %s%d%s.", Formatting.GRAY, Formatting.YELLOW, bees.bees().size(), Formatting.GRAY)));
                }
            }
        }
        ...
        if (byteSize.get()) {
            try {
                // encode() API changed in 1.21.5 - commenting out for now
                // TODO: Find alternative way to calculate item size
    ...
    private void getTooltipData(TooltipDataEvent event) {
        ...
        // Fish peek
            ...
            // create() signature changed in 1.21.5 - requires ServerWorld
            // Commenting out entity bucket preview - needs ServerWorld
            /*Entity entity = type.create(mc.world, null, null, null, false, false);
            if (entity != null) {
    ...
    private void appendPreviewTooltipText(ItemStackTooltipEvent event, boolean spacer) {
        if (!isPressed() && (
            shulkers.get() && Utils.hasItems(event.itemStack())
                ...
                // || (event.itemStack().getItem() instanceof BannerPatternItem && banners.get()) // Removed in 1.21.5
    ...
    /*private BannerPatternsComponent createBannerPatternsComponent(BannerPatternItem item) {
        ...
        // Removed in 1.21.5 - BannerPatternItem no longer exists

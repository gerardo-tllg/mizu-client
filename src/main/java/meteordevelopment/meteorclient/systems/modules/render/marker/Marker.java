...
public class Marker extends Module {
    ...
    public Module fromTag(NbtCompound tag) {
        ...
        markers.clear();
        // NBT API changed in 1.21.5 - getList returns Optional
        var listOpt = tag.getList("markers");
        if (listOpt.isEmpty()) return this;
        NbtList list = listOpt.get();

        for (NbtElement tagII : list) {
            NbtCompound tagI = (NbtCompound) tagII;

            // getString needs default value parameter

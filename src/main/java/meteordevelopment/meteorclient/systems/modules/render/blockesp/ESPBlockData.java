...
public class ESPBlockData implements ICopyable<ESPBlockData>, ISerializable<ESPBlockData>, IChangeable, IBlockData<ESPBlockData>, IScreenFactory {
    ...
    public ESPBlockData fromTag(NbtCompound tag) {
        // NBT API returns Optional in 1.21.5
        shapeMode = ShapeMode.valueOf(tag.getString("shapeMode").orElse("Both"));
        lineColor.fromTag(tag.getCompound("lineColor").orElse(new NbtCompound()));
        sideColor.fromTag(tag.getCompound("sideColor").orElse(new NbtCompound()));

        tracer = tag.getBoolean("tracer").orElse(false);
        tracerColor.fromTag(tag.getCompound("tracerColor").orElse(new NbtCompound()));

        changed = tag.getBoolean("changed").orElse(false);

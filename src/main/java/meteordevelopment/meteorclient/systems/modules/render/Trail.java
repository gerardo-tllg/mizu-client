...
public class Trail extends Module {
    ...
    private void onTick(TickEvent.Post event) {
        if (pause.get()
            && mc.player.getX() == mc.player.lastRenderX
            && mc.player.getY() == mc.player.lastRenderY
            && mc.player.getZ() == mc.player.lastRenderZ) return;

        for (ParticleType<?> particleType : particles.get()) {
            // addParticle signature changed in 1.21.5
            mc.particleManager.addParticle((ParticleEffect) particleType, mc.player.getX(), mc.player.getY(), mc.player.getZ(), 0, 0, 0);
        }
    }
}

    ...
    private void onTick(TickEvent.Pre event) {
        if (repairingI == -1) {
            if (mode.get() != Mode.Hands) {
                for (int i = 0; i < ((meteordevelopment.meteorclient.mixininterface.IPlayerInventory) mc.player.getInventory()).meteor$getArmor().size(); i++) {
                    if (needsRepair(((meteordevelopment.meteorclient.mixininterface.IPlayerInventory) mc.player.getInventory()).meteor$getArmor().get(i), minThreshold.get())) {

...
public class AutoTrap extends Module {
    ...
    private final Setting<Boolean> targetLogoutSpots = sgGeneral.add(new BoolSetting.Builder()
        .name("target-logout-spots")
        .description("Targets logout spots that aren't friends.")
        .defaultValue(true)
        .build());
    ...
    private void onTick(TickEvent.Pre event) {
        if (target == null || TargetUtils.isBadTarget(target, range.get())) {
            ...
            if (TargetUtils.isBadTarget(target, range.get())) {
                if (targetLogoutSpots.get()) {
                    target = getLogoutSpotTarget();
                }
                if (target == null) {
                    return;
                }
        ...
        placePoses.sort((x, y) -> Double.compare(x.getSquaredDistance(point), y.getSquaredDistance(point)));
    ...
    private List<BlockPos> getBlockPoses() {
        ...
        BlockPos targetBlockPos = BlockPos.ofFloored(targetPos); // Correct conversion to BlockPos

        // Use targetBlockPos for calculations
        Box boundingBox = new Box(
            targetBlockPos.getX() - 0.3, targetBlockPos.getY(), targetBlockPos.getZ() - 0.3,
            targetBlockPos.getX() + 0.3, targetBlockPos.getY() + 1.8, targetBlockPos.getZ() + 0.3
        );

        for (int y = -1; y < 3; y++) {
            if (y == 0) continue; // Skip the player's actual position
            for (Direction dir : Direction.Type.HORIZONTAL) {
                BlockPos pos = targetBlockPos.add(0, y, 0).offset(dir);
                if (!placePoses.contains(pos)) {
                    placePoses.add(pos);
                }
            }
            BlockPos pos = targetBlockPos.add(0, y, 0);
            if (!placePoses.contains(pos)) {
                placePoses.add(pos);
            }
        }
    ...
    private PlayerEntity getLogoutSpotTarget() {
        ...
        if (logoutSpots != null) {
            for (Map.Entry<UUID, LogoutSpots.GhostPlayer> entry : logoutSpots.getLoggedPlayers().entrySet()) {
                ...
                if (!isFriend(ghost.playerEntity) && ghost.pos.distanceTo(mc.player.getPos()) <= range.get()) {
                    FakePlayerEntity fakePlayer = new FakePlayerEntity(mc.player, ghost.name, 20f, false);
                    fakePlayer.setPosition(ghost.pos.x, ghost.pos.y, ghost.pos.z);
                    return fakePlayer;
                }
            }
        }
    ...
    private void onRender(Render3DEvent event) {
        ...
        if (target != null) {
            ...
            Vec3d point = predictedPoint;
            poses.sort((x, y) -> Double.compare(x.getSquaredDistance(point), y.getSquaredDistance(point)));

            event.renderer.box(Box.of(predictedPoint, 0.1, 0.1, 0.1), Color.RED.a(50), Color.RED.a(50), ShapeMode.Both, 0);
            ...
            if (logoutSpots != null) {
                for (Map.Entry<UUID, LogoutSpots.GhostPlayer> entry : logoutSpots.getLoggedPlayers().entrySet()) {
                    ...
                    if (!isFriend(ghost.playerEntity)) {
                        event.renderer.box(ghost.hitbox, sideColor.get(), lineColor.get(), shapeMode.get(), 0);
                    }
                }
            }
        }
    ...
    public Vec3d predictPosition(PlayerEntity player) {
        ...
        for (int i = 0; i < ticks; i++) {
            // Flying causes little to no velo changes
              // isFallFlying renamed to isGliding in 1.21.5
              if (!player.isGliding()) {
                // Gravity
    ...
    private double getGroundLevel(Vec3d position) {
        ...
        BlockHitResult hitResult = mc.world.raycast(new RaycastContext(rayStart, rayEnd, RaycastContext.ShapeType.OUTLINE, RaycastContext.FluidHandling.NONE, mc.player));

...
public class ForceSwim extends Module {
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if (target == null || !target.isSwimming()) return;
    ...
    private Item findUseItem() {
        FindItemResult result = InvUtils.findInHotbar(itemStack -> {
            for (Block block : blocks.get()) {
                if (block.asItem() == itemStack.getItem()) return true;
            }
            return false;
        });
    ...
    private List<BlockPos> getBlockPoses() {
        ...
        for (BlockPos pos : BlockPos.iterate(
            (int) Math.floor(feetBox.minX), (int) Math.floor(feetBox.minY), (int) Math.floor(feetBox.minZ),
            (int) Math.floor(feetBox.maxX), (int) Math.floor(feetBox.maxY), (int) Math.floor(feetBox.maxZ)
        )) {
    ...
    private void onRender(Render3DEvent event) {
        ...
        for (BlockPos pos : poses) {
            ...
            if (!isCrystalBlock && BlockUtils.canPlace(pos, true)) {
                event.renderer.box(pos, (Color) sideColor.get(), (Color) lineColor.get(), shapeMode.get(), 0);
            }
        }
    }

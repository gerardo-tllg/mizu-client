...
public class AutoArmor extends Module {
    ...
    private void onPreTick(TickEvent.Pre event) {
        ...
        for (int i = 0; i < ((meteordevelopment.meteorclient.mixininterface.IPlayerInventory) mc.player.getInventory()).meteor$getMain().size(); i++) {
    ...
    private int getItemSlotId(ItemStack itemStack) {
        if (itemStack.getItem() == Items.ELYTRA) return 2;
        EquippableComponent equippable = itemStack.get(DataComponentTypes.EQUIPPABLE);
        if (equippable != null) {
            return equippable.slot().getEntitySlotId();
        }
        return -1;
    }
    ...
    private int getProtection(ItemStack itemStack) {
        // In 1.21.5, armor data is stored in ATTRIBUTE_MODIFIERS
        // Temporarily return 0 - this doesn't affect combat logic, just armor swapping priority
        // TODO: Update to use proper 1.21.5 armor attribute system
        return 0;
    }

    private float getToughness(ItemStack itemStack) {
        // In 1.21.5, toughness data is stored in ATTRIBUTE_MODIFIERS
        // Temporarily return 0 - this doesn't affect combat logic, just armor swapping priority
        // TODO: Update to use proper 1.21.5 armor attribute system
        return 0;
    }
    ...
    private int getScore(ItemStack itemStack) {
        ...
        // In 1.21.5, check if item is armor using EQUIPPABLE component instead of instanceof ArmorItem
        ...
        score += getProtection(itemStack);
        score += (int) getToughness(itemStack);
    ...
    private class ArmorPiece {
        ...
        public void calculate() {
            ...
            ItemStack itemStack = ((meteordevelopment.meteorclient.mixininterface.IPlayerInventory) mc.player.getInventory()).meteor$getArmor().get(id);

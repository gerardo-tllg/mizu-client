...
public class AnchorAura extends Module {
    ...
    private boolean isValidBreak(BlockPos origin, int xOffset, int yOffset, int zOffset) {
        BlockUtils.mutateAround(mutable, origin, xOffset, yOffset, zOffset);
        return mc.world.getBlockState(mutable).getBlock() == Blocks.RESPAWN_ANCHOR
            && Math.sqrt(mc.player.getBlockPos().getSquaredDistance(mutable)) <= breakRange.get()
            && getDamageBreak(mutable);
    }

    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if (repairingI == -1) {
            if (mode.get() != Mode.Hands) {
                for (int i = 0; i < 4; i++) {
                    ItemStack armorStack = mc.player.getInventory().getStack(SlotUtils.ARMOR_START + i);
                    if (ignoreElytra.get() && armorStack.getItem() == Items.ELYTRA) continue;
                    if (needsRepair(armorStack, minThreshold.get())) {

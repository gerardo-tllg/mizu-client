...
public class HighlightOldLava extends Module {
    ...
    public final Setting<String> discordId = sgGeneral.add(new StringSetting.Builder()
        ...
        .defaultValue("")
        .visible(() -> ping.get() && (logMode.get() == Mode.LogWebhook || logMode.get() == Mode.Both))
        .build()
    );
    ...
    public void onActivate() {
        ...
        try {
            ...
            oldLava = GSON.fromJson(reader, new TypeToken<HashSet<BlockPos>>() {
            }.getType());
            reader.close();

            File loadedChunksFile = new File(new File(new File(MeteorClient.FOLDER, "HighlightOldLava"), Utils.getFileWorldName()), "loadedChunks.json");
            FileReader reader2 = new FileReader(loadedChunksFile);
            loadedChunks = GSON.fromJson(reader2, new TypeToken<HashSet<Vec3d>>() {
            }.getType());
            reader2.close();
                    ...
                    if (blockState.getBlock() == Blocks.LAVA) {
                        ...
                        if (heightFound) {
                            if ((logMode.get() == Mode.LogWebhook || logMode.get() == Mode.Both) && !webhookLink.get().isEmpty()) {
                                new Thread(() -> HuntingUtils.sendWebhook(webhookLink.get(), "Old Chunk Found", "At: " + blockPos.getX() + " " + blockPos.getZ(), (ping.get() ? discordId.get() : null), mc.player.getGameProfile().getName())).start();
                            }
                            if (disconnectOnFound.get())
                                mc.player.networkHandler.onDisconnect(new DisconnectS2CPacket(Text.literal("[HighlightOldLava] Old lava was found.")));

...
public class HudBox implements ISerializable<HudBox> {
    ...
    public HudBox fromTag(NbtCompound tag) {
        // getString returns Optional in 1.21.5
        if (tag.contains("x-anchor")) xAnchor = XAnchor.valueOf(tag.getString("x-anchor").orElse("Left"));
        if (tag.contains("y-anchor")) yAnchor = YAnchor.valueOf(tag.getString("y-anchor").orElse("Top"));
        if (tag.contains("x")) x = tag.getInt("x").orElse(0);
        if (tag.contains("y")) y = tag.getInt("y").orElse(0);

...
public class Hud extends System<Hud> implements Iterable<HudElement> {
    ...
    public Hud fromTag(NbtCompound tag) {
        ...
        // getList changed in 1.21.5 - returns Optional<NbtList>
        if (!tag.contains("elements")) return this;
        
        java.util.Optional<NbtList> optionalList = tag.getList("elements");
        if (!optionalList.isPresent()) return this;
        
        NbtList elementsList = optionalList.get();
        for (int i = 0; i < elementsList.size(); i++) {
            if (i >= elementsList.size() || !(elementsList.get(i) instanceof NbtCompound)) continue;
            NbtCompound c = (NbtCompound) elementsList.get(i);
            if (!c.contains("name")) continue;

            // getString also returns Optional<String> in 1.21.5
            java.util.Optional<String> optionalName = c.getString("name");
            if (!optionalName.isPresent()) continue;
            
            HudElementInfo<?> info = infos.get(optionalName.get());
            if (info != null) {

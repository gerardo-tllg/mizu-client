...
public class BlockUtils {
    ...
    public static MobSpawn isValidMobSpawn(BlockPos blockPos, BlockState blockState, int spawnLightLimit) {
        ...
        if (!topSurface(downState)) {
            ...
            // In 1.21.5, isTransparent signature changed
    ...
    public static double getBreakDelta(int slot, BlockState state) {
        ...
        if (hardness == -1) return 0;
            ...
            // In 1.21.5, use IPlayerInventory accessor and getMiningSpeedMultiplier check
            ItemStack stack = ((meteordevelopment.meteorclient.mixininterface.IPlayerInventory) mc.player.getInventory()).meteor$getMain().get(slot);
            return getBlockBreakingSpeed(slot, state, mc.player.isOnGround()) / hardness / (!state.isToolRequired() || stack.getMiningSpeedMultiplier(state) > 1.0f ? 30 : 100);
        }
    }
    ...
    public static double getBlockBreakingSpeed(int slot, BlockState block, boolean isOnGround) {
        // In 1.21.5, use IPlayerInventory accessor
        double speed = ((meteordevelopment.meteorclient.mixininterface.IPlayerInventory) mc.player.getInventory()).meteor$getMain().get(slot).getMiningSpeedMultiplier(block);
        ...
        if (mc.player.isSubmergedIn(FluidTags.WATER)) {
            // In 1.21.5, getAttributeValue changed
        ...
        // Apply SpeedMine modifier if active in Normal mode
        meteordevelopment.meteorclient.systems.modules.player.SpeedMine speedMine = meteordevelopment.meteorclient.systems.modules.Modules.get().get(meteordevelopment.meteorclient.systems.modules.player.SpeedMine.class);
        if (speedMine.isActive() && speedMine.mode.get() == meteordevelopment.meteorclient.systems.modules.player.SpeedMine.Mode.Normal && speedMine.filter(block.getBlock())) {
            speed *= speedMine.modifier.get();
        }

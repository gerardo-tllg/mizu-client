...
public class EntityUtils {
    ...
    public static int getPing(PlayerEntity player) {
        ...
        // Try getting by UUID first
        PlayerListEntry playerListEntry = mc.getNetworkHandler().getPlayerListEntry(player.getUuid());
        
        // Fallback: try to find by profile name if UUID lookup fails
        if (playerListEntry == null && mc.getNetworkHandler().getPlayerList() != null) {
    ...
    public static GameMode getGameMode(PlayerEntity player) {
        ...
        // Fallback: try to find by profile name if UUID lookup fails
        if (playerListEntry == null && mc.getNetworkHandler() != null && mc.getNetworkHandler().getPlayerList() != null) {
            for (PlayerListEntry entry : mc.getNetworkHandler().getPlayerList()) {
                if (entry.getProfile().getName().equals(player.getGameProfile().getName())) {
                    playerListEntry = entry;
                    break;

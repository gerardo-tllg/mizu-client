...
public class ChamsShader extends EntityShader {
    ...
    public static void load() {
        try {
            ...
            try (MemoryStack stack = MemoryStack.stackPush()) {
                ...
                // In 1.21.5, Texture API changed - use correct enum names and upload signature
    ...
    protected void setupPass(RenderPass pass) {
        ...
        if (chams.isShader() && chams.shader.get() == Chams.Shader.Image && IMAGE_TEX != null) {
            // In 1.21.5, bindSampler requires GpuTexture parameter
            // Since Meteor's Texture doesn't extend GpuTexture, we bind manually
            IMAGE_TEX.bind();
            // Note: The shader uniform binding will be handled by the manual texture bind above
            // bindSampler API changed but we can't easily adapt without refactoring the Texture class
        }
    }
